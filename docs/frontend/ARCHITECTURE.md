...
### `app/api/` — route handlers (83 files; mix of own-logic and backend-proxy)
...
## Data & state model (the mental map)
- **Stage** (course) holds **Scenes** ordered, each with a `type` and
...
- **SceneType**: `slide | quiz | interactive | pbl` — each has a dedicated
  renderer in `components/scene-renderers/`.
