 ...
 *
 * Generates a single teacher profile for a course stage
...
function normalizeAgentRole(_role: string): 'teacher' {
  return 'teacher';
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { model: languageModel, modelString: _modelString } = resolveModelForTask(req, 'agent-profiles');
    ...
    const systemPrompt = `You are an expert instructional designer. Generate exactly one teacher profile for a classroom simulation. Return ONLY valid JSON, no markdown or explanation.`;
  ...
  - Pick a voice that suits the teacher persona`
      : '';
...
- Priority value must be 10
- The agent needs: name, role, persona (2-3 sentences describing teaching style)
- Name and persona must be in language: ${language}
- The agent must be assigned one avatar from this list: ${JSON.stringify(avatarDescriptions && avatarDescriptions.length > 0 ? avatarDescriptions.map((a) => ({ path: a.path, description: a.desc })) : availableAvatars)}
  - Pick an avatar that visually matches a teacher persona
  - Use the "path" value as the avatar field in the output
- The agent must be assigned one color from this list: ${JSON.stringify(AGENT_COLOR_PALETTE)}
${voicePrompt}
  ...
  "agents": [
    {
      ...
      "priority": number (must be 10)${voiceJsonField}
    }
  ]
}`;
    ...
    if (!parsed.agents || !Array.isArray(parsed.agents) || parsed.agents.length !== 1) {
      log.error(`Expected exactly 1 agent, got ${parsed.agents?.length ?? 0}`);
      return apiError(
        'GENERATION_FAILED',
        500,
        `Expected exactly 1 teacher agent but LLM returned ${parsed.agents?.length ?? 0}`,
      );
    }

    const teacherCount = parsed.agents.filter((a) => normalizeAgentRole(a.role) === 'teacher').length;
    if (teacherCount !== 1) {
