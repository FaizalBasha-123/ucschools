...
export default function OperatorSchoolsPage() {
  ...
  const createSchool = async () => {
    ...
    try {
      const res = await fetch('/api/operator/schools', {
        ...
        body: JSON.stringify({ name: newName.trim(), operator_email: newEmail.trim(), plan: newPlan }),
      });
  ...
  const fetchMembers = async (schoolId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBaseUrl}/api/operator/schools/${schoolId}/members`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchInvoices = async (schoolId: string) => {
    ...
    try {
      const res = await fetch(`/api/operator/schools/${schoolId}/invoices`);
      if (res.ok) {
        const data = await res.json();
        setInvoices(data || []);
      } else {
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/operator/schools/${selectedSchool.id}/invoices`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'X-Operator-Header': 'true'
          ...
          {showCreate && (
            <div className="rounded-2xl border border-primary/30 bg-white dark:bg-neutral-950 p-6 mb-8 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-xs text-muted-foreground font-medium block mb-1.5">Operator Email</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {schools.map(school => (
                <div key={school.id}
                  ...
                  <p className="text-xs text-muted-foreground mb-4">{school.operator_email}</p>
                ...
                <div className="p-6 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="rounded-xl bg-neutral-50 dark:bg-neutral-900 p-4">
                      ...
                      <p className="font-medium text-sm truncate">{selectedSchool.operator_email}</p>
                    </div>
                  </div>
